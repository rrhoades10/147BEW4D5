...
from flask_sqlalchemy import SQLAlchemy #pip install flask-sqlaclhemy
...
app = Flask(__name__)
cors = CORS(app) #Cross Origin Resrouce Sharing - allows external applications to make requests to our flask app
...
app.json.sort_keys = False #turns key sorting off, fields will matain order set in schema
...
def get_customers():
    ...
    # customer_schema.load(result)
    print(customers) #Fetches all rows of data from the result
...
def updated_customer(customer_id):
    with Session(db.engine) as session: #Creates a session object using the SQLAlchmey Enginge
        with session.begin(): #Begins transaction   
            ...
            return jsonify({"message": "Customer details succesfully updated"}), 200
